...
years <- list(GSW_shots_year_2000, GSW_shots_year_2005, GSW_shots_year_2010, GSW_shots_year_2015, GSW_shots_year_2020, GSW_shots_year_2022)

GSW_shots_combined_years <- rbind(GSW_shots_year_2000, GSW_shots_year_2005, GSW_shots_year_2020, GSW_shots_year_2022)
GSW_shots_change <- rbind(years[[1]],years[[length(years)]])
```

```{r, warning=FALSE, include = FALSE}
#### Understanding the plot of shots for one season for the Golden State Warriors
# Golden State's blue is #1D428A 
# the color for their golden yellow is #FFC72C
first_graph <- ggplot(data=GSW_shots_year_2000, aes(x = locationX, y=locationY - 500)) + 
...
```{r, warning=FALSE}
# court +
#   geom_point(data = GSW_shots_combined_years, aes(x = locationX, y = locationY, color = typeEvent), alpha = 0.1) +
#   scale_color_manual(values = c("#1D428A","#FFC72C")) +
#   labs(x = "", y = "", title = "Missed & Made Shots Throughout the Seasons",color = "Type of Shot") + 
#   facet_wrap(~yearSeason, nrow = 2) + 
#   theme_classic()
court +
  geom_point(data = GSW_shots_combined_years, aes(x = locationX, y = locationY, color = typeEvent), alpha = 0.25) +
  scale_color_manual(values = c("#1D428A","#FFC72C")) +
  labs(x = "", y = "", title = "Missed & Made Shots Throughout {yearSeason}",color = "Type of Shot") + 
  transition_states(yearSeason, transition_length = 0.25, state_length = 2) + 
  enter_fade() +
  exit_shrink() +
  theme_classic()
```
...
```{r}
court +
  geom_point(data = GSW_shots_change, aes(x = locationX, y = locationY, color = typeEvent), alpha = 0.1) +
  scale_color_manual(values = c("#1D428A","#FFC72C")) +
  labs(x = "", y = "", title = "Shooting Change Over the Years",color = "Type of Shot") +
  facet_wrap(yearSeason~typeEvent, nrow = 2) +
  theme_classic()
```
